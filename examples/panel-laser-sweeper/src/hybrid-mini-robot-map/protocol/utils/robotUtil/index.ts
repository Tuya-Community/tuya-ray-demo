...
export function isRobotNotWorking(status: string) {
  const noWorkingStatus = ['standby', 'charging', 'charge_done', 'sleep', 'fault'];
  return noWorkingStatus.includes(status);
}
...
function createFormatPath({ reverseY, hidePath }: { reverseY?: boolean; hidePath?: boolean } = {}) {
  return (originPoint: { x: number; y: number }, type?: number) => {
...
export const isHidePath = createFormatPath({
  reverseY: true,
  hidePath: true,
});
