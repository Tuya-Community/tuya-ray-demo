 ...
 * @private
 */
const uncompress = function(input, output, sIdx, eIdx) {
  sIdx = sIdx || 0;
...
const compressBound = function(isize) {
  return isize > maxInputSize ? 0 : (isize + isize / 255 + 16) | 0;
};

const compress = function(src, dst, sIdx, eIdx) {
...
const compressHC = compress;

const compressDependent = compressBlock;
...
function compressBlock(src, dst, pos, hashTable, sIdx, eIdx) {
  ...
  if (src.length > mfLimit) {
    const n = exports.compressBound(src.length);
