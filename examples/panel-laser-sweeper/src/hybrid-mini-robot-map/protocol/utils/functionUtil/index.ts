...
export function awaitExpectDpsState(
  ...
  return new Promise((resolve, reject) => {
    ...
    const handle = (data: ty.device.DpsChanged) => {
      const dpData = formatDps(data);
      try {
        expectValues.forEach(({ dpCode, value, comparator }) => {
          // 一项dp符合期望时flag+1，flag大于0且不符合期望时-1；直到flag === 期望dp数量
          if (_isUndefined(dpData[dpCode])) return;
          if (_isFunction(comparator)) {
            if (comparator(dpData[dpCode])) flag++;
            if (!comparator(dpData[dpCode]) && flag !== 0) flag--;
          } else {
            if (dpData[dpCode] === value) flag++;
            if (dpData[dpCode] !== value && flag !== 0) flag--;
          }
        });

        if (flag === expectValues.length) {
          resolve(true);
          offDpDataChange(handle);
