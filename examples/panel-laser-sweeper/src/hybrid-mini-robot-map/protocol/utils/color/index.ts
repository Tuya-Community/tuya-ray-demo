import { toFixed16 } from '../stringsUtil';
 ...
 * @returns
 */
function hex2ahexUpcase(colorHex) {
  const color = hex2ahex(colorHex) || '#000000';
 ...
 * @returns
 */
function hex8toHex6(color) {
  return rgba2hex(hex2Rgba(color));
}
 ...
 * @returns
 */
function rgba2hex(originColor) {
  let a,
    rgb = originColor.replace(/\s/g, '').match(/^rgba?\((\d+),(\d+),(\d+),?([^,\s)]+)?/i),
