...
import { isHidePath } from '../utils/robotUtil';
...
const formatPathHeader = (data: string) => {
  ...
  // const [forceUpdate, type] = utils.hexStringToNumber(data.slice(4, 8));
  return {
    version,
    pathId,
    forceUpdate,
...
const decode = memorize((data: string, byteHeaderLength: number = pathHeaderByteLen) => {
  ...
  if (isLz4 && compressAfterLength !== 0) {
    ...
    const encodeDataArray = utils.hexStringToNumber(data.slice(byteHeaderLength));
  ...
  const pathData = pathDataArr.reduce((pre, cur) => {
    const [x, y] = _.chunk(cur, 2).map(([high, low]) => dealPL(utils.highLowToInt(high, low)));
