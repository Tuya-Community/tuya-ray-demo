...
function hex2yuv(hex) {
  const [r, g, b] = decode(hex);
...
function hsb2rgb(h, s, b, a) {
  return hsv2rgb(h, s, b, a);
}
...
function hsv2hex(h, s, v) {
  const rgb = hsv2rgb(h, s, v).map(item => Math.round(item));
  return rgb2hex.apply(this, rgb);
}

function hsb2hex(h, s, b) {
  return hsv2hex(h, s, b);
}
...
function hsl2rgb(h = 0, s = 0, l = 0, a) {
  h = parseFloat(h);
  s = parseFloat(s);
  l = parseFloat(l);
  if (h < 0) h = 0;
...
function complement(color) {
  const rgb = decode(color).map(item => Math.round(item));
  const mm = Math.min(...rgb) + Math.max(...rgb);
  const hex = rgb2hex(...rgb.map(item => mm - item));
  return hex;
}

// 反相
function reversed(color) {
  const rgb = decode(color).map(item => Math.round(item));
  return rgb2hex(...rgb.map(item => maxIn - item));
}

function hex2RgbString(hex, a) {
  const rgb = decode(hex);
...
function hsb2RgbString(...args) {
  return hsv2RgbString.apply(this, args);
}

function hsl2RgbString(h, s, l, a) {
  const rgb = hsl2rgb(h, s, l, a);
  return toRgbString(rgb);
}

function yuv2RgbString(y, u, v, a) {
  const rgb = yuv2rgb(y, u, v, a);
  return toRgbString(rgb);
}

function encodeColorData(rgbhsv) {
...
function decodeColorDataWithPosition(data = '') {
  // rrggbbxxyyvv00
  // xx 0 - 100 x坐标
  // yy 0 - 100 y坐标
  const arr1 = data.match(/[a-z\d]{2}/gi) || [];
...
function encodeColorDataWithPosition(rgbxyve) {
...
function decodeSceneDataWithMode(data) {
  if (!data) {
    return [];
  }
  // l 亮度
 ...
 */
function kelvin2rgbUsingTH(kelvin) {
...
function rgb2kelvin([r, , b]) {
  ...
  while (maxTemperature - minTemperature > epsilon) {
    ...
    const [_r, , _b] = kelvin2rgb(temperature);
    if (_b / _r >= b / r) {
...
const calculateWhiteColorForView = (brightRate, temperatureRate) => {
  ...
  const rgb = hsb2RgbString(hue, sat, 100, alpha);
  return rgb;
};
...
function hsv2rgba(hue, saturation, bright) {
  let res = hsb2hex(hue, saturation / 10, 100);
  res = new ColorObj(color).alpha(bright2Opacity(bright)).rgbString();
  return res;
}

function brightKelvin2rgba(bright, kelvin) {
  let color = brightKelvin2rgb(1000, kelvin || 0);
  color = new ColorObj(color).alpha(bright2Opacity(bright)).rgbString();
  return color;
}
function brightKelvin2rgb(
  bright = 1000,
  kelvin = 1000,
...
function bright2Opacity(brightness, option = { min: 0.2, max: 1 }) {
  const { min = 0.2, max = 1 } = option;
  return Math.round((min + ((brightness - 10) / (1000 - 10)) * (max - min)) * 100) / 100;
}
function temp2rgb(kelvin, { temperatureMin = 4000, temperatureMax = 8000 } = {}) {
  let newKelvin = kelvin;
